...
comp_3.set_ion_properties()
df_sim[comp_3.name]= comp_3.get_df_array()

df_sim.index = ['Radius', 'Length', 'Volume', 'Na_i', 'K_i', 'Cl_i', 'X_i', 'z_i', 'ATP pump rate', 'KCC2 pump rate', 'Vm', 'Ek', 'ECl']
...
while run_t < total_t:
    ...
    for b in range(len(comp_arr)-1):
        ed_conc_changes_arr.append(ed_arr[b].calc_ed(dt,ed_dict_arr[b],ed_dict_arr[b+1])) #makes an array of all the ED conc changes
